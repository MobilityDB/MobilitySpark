...
Sel AS (
  SELECT rows.trip AS trips, rows.licence AS licences,
         rows.vehId AS vehIds, rows.k AS tripIds FROM SelRaw
),
AllRaw AS (
...
AllTrips AS (
  SELECT rows.trip AS trips, rows.vehId AS vehIds,
         rows.k AS tripIds FROM AllRaw
)
